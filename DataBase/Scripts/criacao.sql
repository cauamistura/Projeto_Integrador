...
CREATE TABLE IF NOT EXISTS `DBPI`.`TClinica` (
  ...
  `BDSENHA` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`BDIDCLINICA`),
  INDEX `fk_TClinica_TEndereco1_id` (`BDIDCEP` ASC),
...
CREATE TABLE IF NOT EXISTS `DBPI`.`TAgendamento` (
  `BDIDAGENDAMENTO` INT NOT NULL,
  `BDIDPET` INT NOT NULL,
  `BDDATAAGEN` DATE NOT NULL,
  `BDHORA` varchar(5) NULL,
  PRIMARY KEY (`BDIDAGENDAMENTO`, `BDIDPET`),
  CONSTRAINT `fk_TAgendamento_TPets`
    FOREIGN KEY (`BDIDPET`)
    REFERENCES `DBPI`.`TPets` (`BDIDPET`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION
